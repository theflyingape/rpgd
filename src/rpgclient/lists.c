...
void Lists(void)
{
	static struct RPGmenu RPGmenu = { "Lists", GRN, 5,
								{	{ 'F', "Hall of Fame" },
									{ 'H', "Hi-Scores" },
									{ 'L', "Hall of Lame" },
	...
	while((c = option(&RPGmenu)) != 'Q') {
		switch(c) {
			case 'F':
				while(timeleft() && !cd()) {
					for(o = 0; o < NUMCLASS; o++) {
						sprintf(outbuf, "%s%s %s", bracket(o + 1), fore(GRY), table->class[o]->Origin);
						OUT(outbuf); NL;
					}
					NL;
					...
					for(t = 0; t < table->class[o]->NumCharacter; t++) {
						if(o == 0 && t == 0)
							...
							sprintf(outbuf, "%s%s %s", bracket(t + 1), fore(GRY), table->class[o]->Character[t]->Name);
						OUT(outbuf); NL;
					}
					NL;
					...
					if(o == 0 && t == 0)
						...
						sprintf(line[0], "%s - %s", table->class[o]->Origin, table->class[o]->Character[t]->Name);
					sprintf(line[1], "%*s%s", (int)(22 - strlen(line[0])) / 2, "", line[0]);
			...
			case 'H':
				...
				sprintf(outbuf, "%s%s    The One to Beat%s    %-22s %9.2f %9.2f %srating", bracket(1), fore(CYN), fore(WHITE), line[0], table->HiScore->Rating, PLAYER.Rating, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[1])) / 2, "", table->HiScore->Who[1]);
				sprintf(outbuf, "%s%s  The Sultan of Slash%s  %-22s %9.9g %9lu %shits", bracket(2), fore(CYN), fore(WHITE), line[0], table->HiScore->HP, PLAYER.History.HP, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[2])) / 2, "", table->HiScore->Who[2]);
				sprintf(outbuf, "%s%s The Admirable Admiral%s %-22s %9.9g %9lu %shulls", bracket(3), fore(CYN), fore(WHITE), line[0], table->HiScore->Hull, PLAYER.History.Hull, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[3])) / 2, "", table->HiScore->Who[3]);
				sprintf(outbuf, "%s%s   The Serial Killer%s   %-22s %9.2f %9.2f %skills", bracket(4), fore(CYN), fore(WHITE), line[0], table->HiScore->Kills, (double)PLAYER.History.Kills / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[4])) / 2, "", table->HiScore->Who[4]);
				sprintf(outbuf, "%s%s  Most Popular Player%s  %-22s %9.2f %9.2f %scalls/day", bracket(5), fore(CYN), fore(WHITE), line[0], table->HiScore->Calls, (double)PLAYER.History.Calls / (Julian - PLAYER.CreateDate + 1), fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[5])) / 2, "", table->HiScore->Who[5]);
				sprintf(outbuf, "%s%s  The Easiest Target%s   %-22s %9.2f %9.2f %skilled", bracket(6), fore(CYN), fore(WHITE), line[0], table->HiScore->Killed, (double)PLAYER.History.Killed / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[6])) / 2, "", table->HiScore->Who[6]);
				sprintf(outbuf, "%s%s   The Runaway Train%s   %-22s %9.2f %9.2f %sretreats", bracket(7), fore(CYN), fore(WHITE), line[0], table->HiScore->Retreats, (double)PLAYER.History.Retreats / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[7])) / 2, "", table->HiScore->Who[7]);
				sprintf(outbuf, "%s%s  The Knock-out King%s   %-22s %9.2f %9.2f %sbrawls", bracket(8), fore(CYN), fore(WHITE), line[0], table->HiScore->KO, (double)PLAYER.History.KO / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[8])) / 2, "", table->HiScore->Who[8]);
				sprintf(outbuf, "%s%s  The Joust Champion%s   %-22s %9.2f %9.2f %sjousts", bracket(9), fore(CYN), fore(WHITE), line[0], table->HiScore->Joust, (double)PLAYER.History.Joust / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[9])) / 2, "", table->HiScore->Who[9]);
				sprintf(outbuf, "%s%s The Prince of Thieves%s %-22s %9.2f %9.2f %ssteals", bracket(10), fore(CYN), fore(WHITE), line[0], table->HiScore->Stole, (double)PLAYER.History.Stole / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[10])) / 2, "", table->HiScore->Who[10]);
				sprintf(outbuf, "%s%s  Ace of High Rollers%s  %-22s %9.2f %9.2f %sgambles", bracket(11), fore(CYN), fore(WHITE), line[0], table->HiScore->Gamble, (double)PLAYER.History.Gamble / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL;
				sprintf(line[0], "%*s%s", (int)(22 - strlen(table->HiScore->Who[11])) / 2, "", table->HiScore->Who[11]);
				sprintf(outbuf, "%s%s I'd Rather Be Fishing%s %-22s %9.2f %9.2f %sfishes", bracket(12), fore(CYN), fore(WHITE), line[0], table->HiScore->Fish, (double)PLAYER.History.Fish / PLAYER.History.Calls, fore(CYN));
				OUT(outbuf); NL; NL;
