...
void Editor(struct RPC *rpc,int maxline)
{
	...
	OUT(outbuf);NL;
	if(rpc) {
		sprintf(outbuf,"%s  To:%s %s",fore(CYN),fore(YELLOW),(strlen(rpc->user.Handle) ? (char *)rpc->user.Handle : "All"));
		OUT(outbuf);NL;
	}
...
int ReadMail(char *msg)
{
	...
	if((fp=fopen(msg,"r"))) {
		...
		sprintf(outbuf,"%s  To:%s %s",fore(CYN),fore(YELLOW),strlen(ENEMY.Handle) ? (char *)ENEMY.Handle : "All");
		OUT(outbuf);NL;
		finputs(ENEMY.ID,sizeof(ENEMY.ID),fp);
		RPGserver(SERVER_GETUSER,(UBYTE *)&ENEMY);
