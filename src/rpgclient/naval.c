		...
		switch(c) {
				...
				while((c=option(&SHIPmenu))!='Q')
					switch(c) {
						...
						case 'F':
							...
							OUT(outbuf);NL;
							modf(pow(2.,PLAYER.Hull/150.)*7937.+.5,&cost);
			...
			case 'B':
				...
				if(ONLINE->HULL < 1) {
					...
					OUT(outbuf);
					modf(pow(2., ENEMY.Hull/150.) * 7937. + .5, &cost);
					modf(cost / 250., &cost);
			...
			case 'G':
				...
				if(i<15) {
					...
					if(dice(PLAYER.Level/3+2)==1) {
						...
						for(i=0; i<NUMWEAPON; i++)
							if(!strcasecmp(table->weapon[i]->Origin,"DUNGEON"))
								sprintf(RPC[1][0]->user.Weapon,"%s.%s",table->weapon[i]->Origin,table->weapon[i]->Item[dice(MAXWEAPON(i))-1]->Name);
						sprintf(ENEMY.Armor,"NATURAL.%d",dice(50));
						for(i=0; i<NUMARMOR; i++)
							if(!strcasecmp(table->armor[i]->Origin,"DUNGEON"))
								sprintf(RPC[1][0]->user.Armor,"%s.%s",table->armor[i]->Origin,table->armor[i]->Item[dice(MAXARMOR(i))-1]->Name);
						CreateRPC(RPC[1][0]);
						...
						sprintf(filename,"pix/Naval/%s",ENEMY.Handle);
						type(filename,FALSE);
						sound("god",64,1);
						Delay(50);
						Battle();
						...
						if(ONLINE->HP > 0 && RPC[1][0]->HP > 0) {
							...
							OUT("The old sea hag obliterates your boat in frustration; you drown...");
							Delay(50); NL;
							PLAYER.Hull = 0;
							PLAYER.Cannon = 0;
							PLAYER.Ram = 'N';
				...
				if(i<50) {
					...
					OUT(outbuf);
					ONLINE->HP+=PLAYER.Level+dice(PLAYER.Level)+PLAYER.STR/10+(PLAYER.STR>90 ? PLAYER.STR-90 : 0);
					sprintf(outbuf,"Hit points = %d     ",ONLINE->HP);
					OUT(outbuf);
					if(PLAYER.SP) {
						ONLINE->SP+=PLAYER.Level+dice(PLAYER.Level)+PLAYER.INT/10+(PLAYER.INT>90 ? PLAYER.INT-90 : 0);
				...
				if(i<75) {
					...
					Delay(50);
					modf(pow(2.,PLAYER.Hull/150.)*7937.+.5,&d);
