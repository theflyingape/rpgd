...
void ListGangMembers(struct gang *g1, struct gang *g2, int loaded)
{
	...
	NL;
	sprintf(outbuf, "%s|%s%s%c", fore(GRY), fore(g1->BannerTrim), back(g1->BannerBack), le[g1->BannerType % 7]);
	OUT(outbuf);
	memset(outbuf, tb[g1->BannerType / 8 % 7], 22);
	sprintf(&outbuf[22], "%c%s", re[g1->BannerType % 7], back(BLK));
	OUT(outbuf);
	if(g2) {
		sprintf(outbuf, "   %s|%s%s%c", fore(GRY), fore(g2->BannerTrim), back(g2->BannerBack), le[g2->BannerType % 7]);
		OUT(outbuf);
		memset(outbuf, tb[g2->BannerType / 8 % 7], 22);
		sprintf(&outbuf[22], "%c%s", re[g2->BannerType % 7], back(BLK));
		OUT(outbuf);
	}
	NL;
	...
	sprintf(outbuf, "%s|%s%s%c%s %-20s %s%c%s", fore(GRY), fore(g1->BannerTrim), back(g1->BannerBack), le[g1->BannerType % 7], fore(g1->BannerFore), centered, fore(g1->BannerTrim), re[g1->BannerType % 7], back(BLK));
	OUT(outbuf);
	if(g2) {
		sprintf(centered, "%.20s", g2->Name);
		i = (20 - strlen(centered)) / 2;
		sprintf(centered, "%*s%.20s", i, "", g2->Name);
		sprintf(outbuf, "   %s|%s%s%c%s %-20s %s%c%s", fore(GRY), fore(g2->BannerTrim), back(g2->BannerBack), le[g2->BannerType % 7], fore(g2->BannerFore), centered, fore(g2->BannerTrim), re[g2->BannerType % 7], back(BLK));
		OUT(outbuf);
	}
	NL;
	sprintf(outbuf, "%s|%s%s%c", fore(GRY), fore(g1->BannerTrim), back(g1->BannerBack), le[g1->BannerType % 7]);
	...
	for(i = 0; i < 4; i++) {
		if(loaded) {
			sprintf(outbuf, "%s|%s %-22s", fore(GRY), fore(WHITE), strlen(GANG(0, i).Handle) ?  (char *)GANG(0, i).Handle : "");
			OUT(outbuf);
			sprintf(outbuf, "    %s|%s %-22s", fore(GRY), fore(WHITE), strlen(GANG(1, i).Handle) ? (char *)GANG(1, i).Handle : "");
			OUT(outbuf);
		}
		else {
			...
			sprintf(outbuf, "%s|%s %-22s", fore(GRY), fore(strlen(ENEMY.Handle) ? (strlen(ENEMY.Status) ? BLACK : WHITE) : GRY),strlen(ENEMY.Handle) ? (strncmp(g1->Name, ENEMY.Gang, 20) ? "-reserved-" : (char *)ENEMY.Handle) : "-empty-");
			OUT(outbuf);
			if(g2) {
				...
				if(g2->MemberID[i][0] == '_') {
					...
					RPGserver(SERVER_GETUSER, (UBYTE *)&ENEMY);
				sprintf(outbuf, "    %s|%s %-22s", fore(GRY), fore(strlen(ENEMY.Handle) ? (strlen(ENEMY.Status) ? BLACK : WHITE) : GRY), strlen(ENEMY.Handle) ? (strncmp(g2->Name, ENEMY.Gang, 20) ? "-reserved-" : (char *)ENEMY.Handle) : "-empty-");
				OUT(outbuf);
			}
		}
		NL;
...
void Party(void)
{
	static struct RPGmenu RPGmenu = { "Party", MAG, 8,
					{	{ 'E', "Edit your gang" },
	...
	while((c = option(&RPGmenu)) != 'Q') {
		...
		switch(c) {
			case 'E':
				...
				if(RPGserver(SERVER_GETGANG,(UBYTE *)&gm[0])) {
					...
					if(ACCESS(acclvl)->RolePlay=='Y') {
						sprintf(prompt,"%sReplace member (2-4 or <C>olors): ",fore(CYN));
						OUT(prompt);
						if(ins(4))
							break;
					}
					...
					if(strlen(inbuf))
						for(i=1; i<4 && strcasecmp(gm[0].MemberID[i],inbuf); i++);
					if(i>0 && i<4) {
			...
			case 'F':
				...
				for(i=0; i<4; i++)
					if(strlen(gm[0].MemberID[i]) && strcmp(gm[0].MemberID[i],PLAYER.ID)) {
						...
						if(RPGserver(SERVER_GETUSER,(UBYTE *)&GANG(0,j))) {
							if(!strncmp(gm[0].Name, GANG(0,j).Gang, sizeof(GANG(0,j).Gang)) && !strlen(GANG(0,j).Status) && RPGserver(SERVER_LOCK, (UBYTE *)GANG(0,j).ID)) {
								LoadRPC(RPC[0][j]);
				...
				for(i=0; i<4; i++)
					if(strlen(gm[1].MemberID[i])) {
						...
						if(GANG(1,j).ID[0]=='_') {
							...
							GANG(1,j).Gender='I';
							GANG(1,j).Level=l+1;
							modf(pow(2.,l/2.)*dice(10),&GANG(1,j).Gold);
							CreateRPC(RPC[1][j]);
							if(!RPC[1][j]->class_origin && !RPC[1][j]->class_type) {
								w=dice(NUMCLASS)-1;
								sprintf(GANG(1,j).Class,"%s.%s",table->class[w]->Origin,table->class[w]->Character[dice(MAXCLASS(w))-1]->Name);
								CreateRPC(RPC[1][j]);
							}
							for(w=0; w<NUMMAGIC; w++)
								if(dice(ONLINE->CHA/2+5*w)==1) {
									if(w<16)
										GANG(1,j).Spell|=(UWORD)pow(2.,(double)w);
									if(w>15 && w<24)
										GANG(1,j).XSpell|=(UBYTE)pow(2.,(double)w-16);
								}
							for(w=0; w<NUMPOISON; w++)
								if(dice(ONLINE->CHA/(RPC[1][j]->user.MyPoison+w+1))==1)
									GANG(1,j).Poison|=(UWORD)pow(2.,(double)w);
						...
						if(RPGserver(SERVER_GETUSER,(UBYTE *)&GANG(1,j))) {
							if(!strncmp(gm[1].Name, GANG(1,j).Gang, sizeof(GANG(1,j).Gang)) && !strlen(GANG(1,j).Status) && RPGserver(SERVER_LOCK, (UBYTE *)GANG(1,j).ID)) {
								LoadRPC(RPC[1][j]);
								divisor[1]+=pow(2.5,GANG(1,j).ExpLevel/3.);
								totgold[1]+=GANG(1,j).Gold;
								totlvl[1]+=GANG(1,j).ExpLevel;
				...
				for(i=0; i<4; i++) {
					...
					if(strlen(GANG(w,i).ID) && GANG(w,i).ID[0]!='_' && RPC[w][i]->HP>-1) {
						modf(pow(2.,(GANG(w,i).Level-2.))*1000.*totlvl[l]/totlvl[w],&expaward);
			...
			case 'M':
				cls();
				sprintf(outbuf, "%s%s       Party            Win-Loss   Ratio", back(BLU), fore(WHITE));
				OUT(outbuf); NORMAL; NL;
				sprintf(outbuf, "%s%s----------------------------------------", back(BLU), fore(GRY));
				OUT(outbuf); NORMAL; NL;
				i = 0;
				memset(&gm[0], 0, sizeof(struct gang));
				while(RPGserver(SERVER_NEXTGANG, (UBYTE *)&gm[0]) && i < 50) {
					// check to see if this gang qualifies to go on the board...
					if(gm[0].Win < 10 && gm[0].Loss < 10)
						continue;
					memcpy(&gm[2], &gm[0], sizeof(struct gang));
					sprintf(line[i], "%.20s", gm[0].Name);
					divisor[0] = (double)gm[0].Win / (gm[0].Win+gm[0].Loss);
					j = i;
					while(j > 0) {
						memcpy(gm[1].Name, line[j-1], 20);
						RPGserver(SERVER_GETGANG, (UBYTE *)&gm[1]);
						divisor[1] = (double)gm[1].Win / (gm[1].Win + gm[1].Loss);
						if(divisor[0] < divisor[1])
							break;
						if(divisor[0] == divisor[1] && gm[0].Loss < gm[1].Loss)
							break;
			...
			case 'S':
				...
				NL;NL;
				proper(inbuf);
				if(!strlen(inbuf) || !strcasecmp(inbuf,"None"))
					break;
